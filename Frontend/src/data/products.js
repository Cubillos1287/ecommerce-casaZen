import img1 from "../assets/imgs/1.jpg";
import img2 from "../assets/imgs/2.jpg";
import img3 from "../assets/imgs/3.jpg";
import img4 from "../assets/imgs/4.jpg";
import img5 from "../assets/imgs/5.jpg";
import img6 from "../assets/imgs/6.jpg";
import img7 from "../assets/imgs/7.jpg";
import img8 from "../assets/imgs/8.jpg";
import img9 from "../assets/imgs/9.jpg";
import img10 from "../assets/imgs/10.jpg";







export const productos = [
  // ================== COCINA ==================
  {
    id: 1,
    categoria: "cocina",
    nombre: "Tabla de corte de bambú",
    precio: 18.99,
    descripcion: "Resistente y ecológica, ideal para una cocina moderna.",
    img: img1
  },
  {
    id: 2,
    categoria: "cocina",
    nombre: "Set de utensilios de madera",
    precio: 24.99,
    descripcion: "Cocina con estilo natural todos los días.",
    img: img2
  },
  {
    id: 3,
    categoria: "cocina",
    nombre: "Frascos de vidrio hermético",
    precio: 9.99,
    descripcion: "Mantén tus alimentos frescos y ordenados.",
    img: img3
  },
  {
    id: 4,
    categoria: "cocina",
    nombre: "Porta especias minimalista",
    precio: 14.99,
    descripcion: "Organización práctica con diseño elegante.",
    img: img4
  },
  {
    id: 5,
    categoria: "cocina",
    nombre: "Tazas de cerámica",
    precio: 12.99,
    descripcion: "Perfecta para disfrutar tu café favorito.",
    img: img5
  },
  {
    id: 6,
    categoria: "cocina",
    nombre: "Plato decorativo blanco",
    precio: 15.99,
    descripcion: "Un toque limpio y moderno para tu mesa.",
    img: img6
  },
  {
    id: 7,
    categoria: "cocina",
    nombre: "Escurridor metálico",
    precio: 22.99,
    descripcion: "Funcional y elegante para tu fregadero.",
    img: img7
  },
  {
    id: 8,
    categoria: "cocina",
    nombre: "Botella de vidrio reutilizable",
    precio: 11.99,
    descripcion: "Ideal para hidratarte con estilo.",
    img: img8
  },
  {
    id: 9,
    categoria: "cocina",
    nombre: "Paños de cocina de algodón",
    precio: 10.99,
    descripcion: "Suaves, prácticos y con diseño moderno.",
    img: img9
  },
  {
    id: 10,
    categoria: "cocina",
    nombre: "Bandeja de madera",
    precio: 19.99,
    descripcion: "Perfecta para servir o decorar.",
    img: img10
  },

  // ================== BAÑO ==================
  {
    id: 11,
    categoria: "baño",
    nombre: "Dispensador de jabón",
    precio: 13.99,
    descripcion: "Dale un toque moderno a tu baño.",
    img: img3
  },
  {
    id: 12,
    categoria: "baño",
    nombre: "Porta cepillos de cerámica",
    precio: 11.99,
    descripcion: "Orden y estilo en tu lavamanos.",
    img: img5
  },
  {
    id: 13,
    categoria: "baño",
    nombre: "Set de toallas blancas",
    precio: 29.99,
    descripcion: "Suavidad y confort tipo hotel.",
    img: img8
  },
  {
    id: 14,
    categoria: "baño",
    nombre: "Bandeja organizadora",
    precio: 14.99,
    descripcion: "Ideal para perfumes y accesorios.",
    img: img4
  },
  {
    id: 15,
    categoria: "baño",
    nombre: "Cortina de baño neutra",
    precio: 21.99,
    descripcion: "Combina fácilmente con cualquier estilo.",
    img: img9
  },
  {
    id: 16,
    categoria: "baño",
    nombre: "Alfombra antideslizante",
    precio: 18.99,
    descripcion: "Seguridad y comodidad al salir de la ducha.",
    img: img2
  },
  {
    id: 17,
    categoria: "baño",
    nombre: "Cesto para ropa",
    precio: 26.99,
    descripcion: "Mantén el orden sin perder diseño.",
    img: img6
  },
  {
    id: 18,
    categoria: "baño",
    nombre: "Frasco de vidrio para algodón",
    precio: 9.99,
    descripcion: "Pequeño detalle que marca la diferencia.",
    img: img3
  },
  {
    id: 19,
    categoria: "baño",
    nombre: "Porta papel higiénico",
    precio: 15.99,
    descripcion: "Diseño simple y funcional.",
    img: img7
  },
  {
    id: 20,
    categoria: "baño",
    nombre: "Espejo redondo",
    precio: 39.99,
    descripcion: "Aporta amplitud y elegancia.",
    img: img10
  },

  // ================== OFICINA ==================
  {
    id: 21,
    categoria: "oficina",
    nombre: "Organizador de escritorio",
    precio: 17.99,
    descripcion: "Mantén tu espacio limpio y ordenado.",
    img: img4
  },
  {
    id: 22,
    categoria: "oficina",
    nombre: "Lámpara LED de escritorio",
    precio: 34.99,
    descripcion: "Iluminación ideal para trabajar o estudiar.",
    img: img8
  },
  {
    id: 23,
    categoria: "oficina",
    nombre: "Soporte para laptop",
    precio: 29.99,
    descripcion: "Mejora tu postura mientras trabajas.",
    img: img2
  },
  {
    id: 24,
    categoria: "oficina",
    nombre: "Cuaderno minimalista",
    precio: 12.99,
    descripcion: "Perfecto para notas y planificación.",
    img: img1
  },
  {
    id: 25,
    categoria: "oficina",
    nombre: "Portalápices metálico",
    precio: 9.99,
    descripcion: "Diseño simple y funcional.",
    img: img4
  },
  {
    id: 26,
    categoria: "oficina",
    nombre: "Reloj de escritorio",
    precio: 19.99,
    descripcion: "Controla tu tiempo con estilo.",
    img: img5
  },
  {
    id: 27,
    categoria: "oficina",
    nombre: "Mouse pad neutro",
    precio: 8.99,
    descripcion: "Comodidad para largas jornadas.",
    img: img9
  },
  {
    id: 28,
    categoria: "oficina",
    nombre: "Archivador de cartón",
    precio: 14.99,
    descripcion: "Organiza tus documentos fácilmente.",
    img: img6
  },
  {
    id: 29,
    categoria: "oficina",
    nombre: "Soporte para celular",
    precio: 10.99,
    descripcion: "Ideal para videollamadas.",
    img: img3
  },
  {
    id: 30,
    categoria: "oficina",
    nombre: "Pizarra pequeña",
    precio: 16.99,
    descripcion: "Visualiza tus tareas diarias.",
    img: img7
  },

  // ================== DECORACIÓN ==================
  {
    id: 31,
    categoria: "decoracion",
    nombre: "Jarrón de cerámica",
    precio: 24.99,
    descripcion: "Un detalle elegante para cualquier espacio.",
    img: img10
  },
  {
    id: 32,
    categoria: "decoracion",
    nombre: "Vela aromática",
    precio: 14.99,
    descripcion: "Crea un ambiente cálido y relajante.",
    img: img5
  },
  {
    id: 33,
    categoria: "decoracion",
    nombre: "Marco para fotos",
    precio: 12.99,
    descripcion: "Resalta tus recuerdos favoritos.",
    img: img2
  },
  {
    id: 34,
    categoria: "decoracion",
    nombre: "Reloj de pared moderno",
    precio: 29.99,
    descripcion: "Diseño limpio y actual.",
    img: img9
  },
  {
    id: 35,
    categoria: "decoracion",
    nombre: "Bandeja decorativa",
    precio: 18.99,
    descripcion: "Versátil y sofisticada.",
    img: img4
  },
  {
    id: 36,
    categoria: "decoracion",
    nombre: "Planta artificial",
    precio: 21.99,
    descripcion: "Toque verde sin mantenimiento.",
    img: img1
  },
  {
    id: 37,
    categoria: "decoracion",
    nombre: "Cojín decorativo",
    precio: 19.99,
    descripcion: "Confort y estilo para tu sofá.",
    img: img8
  },
  {
    id: 38,
    categoria: "decoracion",
    nombre: "Figura decorativa",
    precio: 16.99,
    descripcion: "Un detalle moderno y sutil.",
    img: img6
  },
  {
    id: 39,
    categoria: "decoracion",
    nombre: "Espejo decorativo",
    precio: 34.99,
    descripcion: "Elegancia para cualquier pared.",
    img: img10
  },
  {
    id: 40,
    categoria: "decoracion",
    nombre: "Canasta de mimbre",
    precio: 27.99,
    descripcion: "Orden con estilo natural.",
    img: img3
  },

  // ================== CUARTO ==================
  {
    id: 41,
    categoria: "cuarto",
    nombre: "Lámpara de mesa",
    precio: 28.99,
    descripcion: "Iluminación cálida para tu descanso.",
    img: img5
  },
  {
    id: 42,
    categoria: "cuarto",
    nombre: "Juego de sábanas",
    precio: 49.99,
    descripcion: "Suavidad que se siente al dormir.",
    img: img9
  },
  {
    id: 43,
    categoria: "cuarto",
    nombre: "Almohada ergonómica",
    precio: 32.99,
    descripcion: "Descanso cómodo toda la noche.",
    img: img8
  },
  {
    id: 44,
    categoria: "cuarto",
    nombre: "Manta decorativa",
    precio: 26.99,
    descripcion: "Calidez y estilo para tu cama.",
    img: img7
  },
  {
    id: 45,
    categoria: "cuarto",
    nombre: "Mesa de noche",
    precio: 59.99,
    descripcion: "Funcional y moderna.",
    img: img6
  },
  {
    id: 46,
    categoria: "cuarto",
    nombre: "Organizador de clóset",
    precio: 22.99,
    descripcion: "Aprovecha mejor tu espacio.",
    img: img4
  },
  {
    id: 47,
    categoria: "cuarto",
    nombre: "Espejo de piso",
    precio: 69.99,
    descripcion: "Elegancia y amplitud visual.",
    img: img10
  },
  {
    id: 48,
    categoria: "cuarto",
    nombre: "Perchero minimalista",
    precio: 24.99,
    descripcion: "Orden práctico y decorativo.",
    img: img2
  },
  {
    id: 49,
    categoria: "cuarto",
    nombre: "Cesto decorativo",
    precio: 19.99,
    descripcion: "Ideal para ropa o mantas.",
    img: img3
  },
  {
    id: 50,
    categoria: "cuarto",
    nombre: "Cuadro decorativo",
    precio: 29.99,
    descripcion: "Dale personalidad a tu habitación.",
    img: img9
  }
];

export default productos;